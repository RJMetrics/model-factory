...
(function() {
  ...
  describe('Model Factory', function() {
    ...
    return it("@query properly queries the endpoint and stores the resulting models in the cache and in the _queryMap", function() {
      var Model, modelData, promise, queryResult;
      modelData = {
        id: 1,
        name: "model5"
      };
      httpBackend.expectGET(new RegExp("" + modelUrl + "\\?name=model5")).returns(modelData);
      queryResult = null;
      Model = modelFactory(modelUrl);
      promise = Model.query({
        name: "model5"
      ...
      return expect(queryResult.length).toBe(1);
    });
  });
